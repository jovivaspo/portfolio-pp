...
const listOfVideos = [LogoAnimationWatter, LogoAnimationPaint, LogoAnimationBubbles];
...
export const LogoAnimated = () => {
  ...
  useEffect(() => {
    ...
    $video.addEventListener('ended', () => {
      setVideoIndex(videosIndex === listOfVideos.length - 1 ? 0 : videosIndex + 1);
      setIsPlaying(false);
    });
  }, [video]);
    ...
    <Link to='/'>
      ...
      >
        <div className='image__overlay_logo w-full'>
          <img src={Logo} alt='Logo' className='opacity-100 w-full h-full object-cover bg-transparent' />
        </div>
