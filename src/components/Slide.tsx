...
export const Slide = () => {
 ...
 function ConsoleText({ words }: { words: string[]}) {
    ...
    const interval2 = window.setInterval(() => {
      if (conRef.current) {
        if (visible) {
          conRef.current.className = 'text-my-gray inline-block relative top-[-0.14em] left-[10px] opacity-0';
           visible = false;
        } else {
          conRef.current.className = 'text-my-gray inline-block relative top-[-0.14em] left-[10px]';
          visible = true;
        }
      }
    }, 400);
  ...
  return (
    <div className="w-full h-[300px] flex flex-col justify-center items-center gap-10">
      ...
      <div className='text-center'>
        ...
        <div ref={conRef} className='console-underscore' id='console'>&#95;</div>
      </div>
