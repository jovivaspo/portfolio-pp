    ...
    <footer className='w-full min-h-[200px] bg-white flex flex-col gap-16 pt-[40px] md:mb-12'>
      <ul className='w-full flex justify-center gap-5'>
        {IconsMenu.map((item, index) => {
          return (
            <li className='w-[30px] h-[30px] rounded-full flex justify-center items-center transition-all hover:scale-110' key={index}>
      ...
      <ul className='w-full flex flex-col items-center justify-center gap-8 p-4 md:flex-row'>
        {menuFooter.map((item, index) => {
          return (
            <li key={index}>
              <Link className='text-sm  sub-menu relative cursor-pointer text-black hover:text-my-gray transition-colors duration-500 ease-in-out' to={item.path}>
